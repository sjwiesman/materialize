...
see [Retain history](/model-data/patterns/durable-subscriptions/#set-history-retention-period).
{{</ tip >}}
...
see [Partitioning and filter pushdown](/model-data/patterns/partition-by/).
{{</ tip >}}
...
pushdown](/model-data/patterns/partition-by/#filter-pushdown) is effective
...
[`PARTITION BY` requirements](/model-data/patterns/partition-by/#requirements)
for the full list.
...
materialized views using any common PostgreSQL-compatible [client library](/interfaces/client-libraries/)
and [`SUBSCRIBE`](/sql/subscribe/)(see the [Python cheatsheet](/interfaces/client-libraries/python/#stream)
