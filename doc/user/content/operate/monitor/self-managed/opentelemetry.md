...
1. Supply the credential. Two inputs carry credentials, and they are mutually
   ...
   | Input | Use when |
   |-------|----------|
   | `otlp_auth_header_secrets` | The destination authenticates with an API-key header. This is how most OTLP vendors work. See [Honeycomb](/operate/monitor/self-managed/honeycomb/) for a worked example. |
