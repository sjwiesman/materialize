...
- **`dependencies`** — external dependency declarations for objects your project
  references but does not own. See [Local development](/operate/mz-deploy/local-development/)
  for details.
- **Per-profile config sections** — override settings for specific environments.
  See [Profiles](/operate/mz-deploy/profiles/) for multi-environment setup.
...
set`. See [Profiles](/operate/mz-deploy/profiles/).
...
See [Profiles](/operate/mz-deploy/profiles/) for multi-environment setup and
