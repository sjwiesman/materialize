...
* Make the syntax for [`GROUP SIZE` query hints](/model-data/optimization/#query-hints)
...
* Add `savings` to the [`mz_internal.mz_expected_group_size_advice`](/reference/system-catalog/mz_introspection/#mz_expected_group_size_advice)
  ...
  savings that can be expected by using [`GROUP SIZE` query hints](/model-data/optimization/#query-hints).
