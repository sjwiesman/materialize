...
* **Private preview.** Support setting a history retention period for sources,
    tables, materialized views, and indexes via the new [`RETAIN HISTORY`](/model-data/patterns/durable-subscriptions/#history-retention-period)
    option. This is useful to implement [durable subscriptions](/model-data/patterns/durable-subscriptions/).
