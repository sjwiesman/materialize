...
{{< warning >}}
`EXECUTE UNIT TEST` is executed only by [`mz-deploy`](/operate/mz-deploy/), not by
...
[`mz-deploy test`](/operate/mz-deploy/local-development/#write-and-run-unit-tests)
...
`EXPECTED` column list. Run [`mz-deploy lock`](/operate/mz-deploy/local-development/#lock-types)
...
- [Local development with mz-deploy](/operate/mz-deploy/local-development/#write-and-run-unit-tests)
