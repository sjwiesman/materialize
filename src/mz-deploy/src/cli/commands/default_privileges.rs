...
mod tests {
    ...
    /// A revoke clause mis-pluralizes the same way a grant clause does.
    #[mz_ore::test]
    #[cfg_attr(miri, ignore)] // unsupported operation: can't call foreign function 'rust_psm_stack_pointer'
    fn test_stale_network_policy_rule_is_revoked() {
        assert_eq!(
            changes(
                &[],
                vec![row("owner", "network policy", "analyst", "USAGE")],
            ),
            vec![
                "ALTER DEFAULT PRIVILEGES FOR ROLE owner IN SCHEMA db.app \
                 REVOKE USAGE ON POLICIES FROM analyst"
            ]
        );
    }
