...
mod tests {
    ...
    use crate::project::ir::graph::{Database, DatabaseObject, Project, Schema, SchemaType};
    use crate::project::ir::object_id::ObjectId;
    ...
    fn test_index_cluster_does_not_dirty_parent_object_cluster() {
        ...
        let base_facts = BaseFacts {
            ...
            depends_on: vec![],
            stmt_uses_cluster: vec![
                (mv.clone(), "staging".into()),
                (other.clone(), "quickstart".into()),
            ],
            index_uses_cluster: vec![(mv.clone(), "idx_item".to_string(), "quickstart".into())],
            is_sink: BTreeSet::new(),
            is_replacement: BTreeSet::new(),
        };
    ...
    fn test_index_cluster_does_not_dirty_schema() {
        ...
        let base_facts = BaseFacts {
            ...
            depends_on: vec![],
            stmt_uses_cluster: vec![(other.clone(), "index_cluster".into())],
            index_uses_cluster: vec![(table1.clone(), "idx1".to_string(), "index_cluster".into())],
            is_sink: BTreeSet::new(),
            is_replacement: BTreeSet::new(),
        };
    ...
    fn test_dependency_propagation_with_index_cluster_conflict() {
        ...
        let base_facts = BaseFacts {
            ...
            stmt_uses_cluster: vec![(winning_bids.clone(), "staging".into())],
            index_uses_cluster: vec![
                (
                    winning_bids.clone(),
                    "idx_item".to_string(),
    ...
    fn test_replacement_mv_does_dirty_its_cluster() {
        ...
        let base_facts = BaseFacts {
            ...
            depends_on: vec![],
            stmt_uses_cluster: vec![(mv1.clone(), "analytics_cluster".into())],
            index_uses_cluster: vec![],
            is_sink: BTreeSet::new(),
            is_replacement,
        };
    ...
    fn test_replacement_mv_dirty_via_cluster() {
        ...
        let base_facts = BaseFacts {
            ...
            depends_on: vec![],
            stmt_uses_cluster: vec![
                (regular.clone(), "shared_cluster".into()),
                (replacement_mv.clone(), "shared_cluster".into()),
            ],
            index_uses_cluster: vec![],
            is_sink: BTreeSet::new(),
            is_replacement,
    ...
    fn test_mixed_replacement_and_regular_in_shared_cluster() {
        ...
        let base_facts = BaseFacts {
            ...
            depends_on: vec![],
            stmt_uses_cluster: vec![
                (regular_view.clone(), "shared".into()),
                (replacement_mv1.clone(), "shared".into()),
                (replacement_mv2.clone(), "shared".into()),
            ],
            index_uses_cluster: vec![],
            is_sink: BTreeSet::new(),
            is_replacement,
