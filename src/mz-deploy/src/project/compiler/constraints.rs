...
//! Whole-project constraint validation.
//!
//! These validators run after type checking when column information is
//! available, and they're consumed both inside the compiler (from
//! `compiler.rs`-driven flows) and from the CLI (`cli/commands/compile.rs`),
//! so they live at the compiler module level rather than inside
//! `object_validation/`.
