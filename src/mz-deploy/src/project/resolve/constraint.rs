...
//! # Graph integration
//!
//! Lowering happens during project-graph assembly in
//! [`crate::project::analysis::deps`]. The companion MVs are injected as
//! first-class graph objects, so they automatically participate in dependency
//! tracking, topological sorting, change detection, staging normalization, and
...
/// it produces a `compiled::DatabaseObject` containing a `CREATE MATERIALIZED VIEW`
...
pub fn lower_to_materialized_view(
    ...
    database: &str,
    schema: &str,
) -> Option<compiled::DatabaseObject> {
...
mod tests {
    ...
    fn lowered_query_sql(obj: &compiled::DatabaseObject) -> String {
        match &obj.stmt {
    ...
    fn lowered_mv_name(obj: &compiled::DatabaseObject) -> String {
        match &obj.stmt {
