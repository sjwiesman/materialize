//! Source-owned project inputs discovered from disk.
//!
//! This module loads and parses Materialize project files into source-facing
//! structures that mirror the directory layout and retain profile-specific file
//! variants. These types carry parsed SQL and source locations but do not
//! enforce semantic object invariants.
//!
//! # Directory Structure
//!
...
//!
//! These files are parsed and stored as source inputs. They participate in
//! project assembly and validation but are not compiled into per-object SQL artifacts.

use crate::project::error::{LoadError, ProjectError};
use crate::project::syntax::parser::{LocatedStatement, parse_statements_with_context};
...
/// The complete source-owned project structure loaded from the file system.
///
/// Represents the entire project directory tree with all databases, schemas, and
/// objects loaded and parsed but not yet validated. This is the top-level source input
...
///
/// Loading is structured as collect → process → reassemble:
///
...
pub fn load_project<P: AsRef<Path>>(
    ...
    crate::timing!("  input: reassemble", reassemble_start.elapsed());
...
mod tests {
    ...
    fn test_cluster_dependencies_through_full_pipeline() {
        ...
        let planned_project =
            compiler::compile_sync(root, "default", None, &BTreeMap::new()).unwrap();
