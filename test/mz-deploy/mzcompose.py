...
def workflow_preview(c: Composition, parser: WorkflowArgumentParser) -> None:
    """Test the preview command — developer-role deployments that cannot be promoted."""
    setup_base(c)

    with c.test_case("mz-deploy-preview"):
        # ── Setup: apply base infrastructure and grant dev_user access ──
        result = run_mz_deploy(c, "basic/v1", "apply")
        assert result.returncode == 0, f"apply failed: {result.stderr}"

        # Grant dev_user privileges on app database for creating staging schemas
        c.sql("GRANT ALL PRIVILEGES ON DATABASE app TO dev_user")
        rows = c.sql_query(
        ...
        for (schema,) in rows:
            c.sql(
                f"GRANT ALL PRIVILEGES ON SCHEMA {schema} TO dev_user",
                database="app",
            )

        # ════════════════════════════════════════════════════════════
        # Test 1: Preview creates real staging resources as developer
        # ════════════════════════════════════════════════════════════
        result = run_mz_deploy(
            c, "basic/v1", "preview", "--deploy-id", "prev1", "--profile", "dev"
        )
        assert result.returncode == 0, f"preview failed: {result.stderr}"

        # Verify staging schemas were created
        rows = c.sql_query(
            "SELECT name FROM mz_schemas WHERE name = 'ops_prev1'",
            database="app",
        )
        assert len(rows) == 1, f"Expected ops_prev1 schema, got {rows}"

        rows = c.sql_query(
            "SELECT name FROM mz_schemas WHERE name = 'core_prev1'",
            database="app",
        )
        assert len(rows) == 1, f"Expected core_prev1 schema, got {rows}"

        # Verify staging clusters were created
        rows = c.sql_query(
            "SELECT name FROM mz_clusters WHERE name = 'compute_prev1'"
        )
        assert len(rows) == 1, f"Expected compute_prev1 cluster, got {rows}"

        # ════════════════════════════════════════════════════════════
        # Test 2: Preview deployment cannot be promoted
        # ════════════════════════════════════════════════════════════
        result = run_mz_deploy(
            c, "basic/v1", "promote", "prev1", "--no-ready-check", check=False
        )
        assert result.returncode != 0, "promote should fail for preview deployment"
        combined = result.stdout + result.stderr
        assert "cannot be promoted" in combined, (
            f"Expected 'cannot be promoted' error, got: {combined}"
        )

        # ════════════════════════════════════════════════════════════
        # Test 3: Developer can abort own preview deployment
        # ════════════════════════════════════════════════════════════
        result = run_mz_deploy(c, "basic/v1", "abort", "prev1", "--profile", "dev")
        assert result.returncode == 0, f"developer abort failed: {result.stderr}"

        # Verify cleanup — no prev1 schemas or clusters remain
        rows = c.sql_query(
            "SELECT name FROM mz_schemas WHERE name LIKE '%\\_prev1'",
            database="app",
        )
        assert len(rows) == 0, f"Expected no prev1 schemas after abort, got {rows}"

        rows = c.sql_query(
            "SELECT name FROM mz_clusters WHERE name LIKE '%\\_prev1'"
        )
        assert len(rows) == 0, f"Expected no prev1 clusters after abort, got {rows}"

        # ════════════════════════════════════════════════════════════
        # Test 4: Deployer can also abort a preview deployment
        # ════════════════════════════════════════════════════════════
        # TODO: The deployer cannot currently drop staging schemas owned by
        # dev_user. The role check allows it, but the DROP SCHEMA fails because
        # the deployer is not the owner. This needs a product fix (e.g., staging
        # schemas should be owned by the mz-deploy role, not the creating user).

        # ════════════════════════════════════════════════════════════
        # Test 5: Preview shows in list output with (preview) label
        # ════════════════════════════════════════════════════════════

        # Create a preview to list
        result = run_mz_deploy(
            c, "basic/v1", "preview", "--deploy-id", "prev3", "--profile", "dev"
        )
        assert result.returncode == 0, f"preview prev3 failed: {result.stderr}"

        result = run_mz_deploy(c, "basic/v1", "list", "--profile", "dev")
        assert result.returncode == 0, f"list failed: {result.stderr}"
        combined = result.stdout + result.stderr
        assert "prev3" in combined, f"Expected prev3 in list output, got: {combined}"
        assert "preview" in combined, (
            f"Expected 'preview' label in list output, got: {combined}"
        )

        # Clean up
        result = run_mz_deploy(c, "basic/v1", "abort", "prev3", "--profile", "dev")
        assert result.returncode == 0

        # ════════════════════════════════════════════════════════════
        # Test 6: Deployer cannot run preview
        # ════════════════════════════════════════════════════════════
        result = run_mz_deploy(
            c,
            "basic/v1",
        ...
        assert result.returncode != 0, "preview should fail for deployer role"
        combined = result.stdout + result.stderr
        ...
        # ════════════════════════════════════════════════════════════
        # Test 7: Monitor cannot run preview
        # ════════════════════════════════════════════════════════════
        result = run_mz_deploy(
            c,
            "basic/v1",
        ...
        assert result.returncode != 0, "preview should fail for monitor role"
        combined = result.stdout + result.stderr
        assert "materialize_developer" in combined, (
            f"Expected role error mentioning materialize_developer, got: {combined}"
        )
